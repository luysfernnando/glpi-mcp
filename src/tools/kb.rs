...
pub struct SearchKbArticlesParams {
    ...
    pub range_limit: i64,
    #[schemars(
        description = "Also match against the article body; only enable when the GLPI DB has a FULLTEXT index on knowbaseitems.answer, otherwise the request is slow"
    )]
    #[serde(default)]
...
fn render_kb_list(items: &[Value]) -> String {
    ...
    let rows: Vec<Vec<String>> = items
        ...
        .map(|item| {
            ...
            let faq = if item.get("is_faq").and_then(Value::as_i64).unwrap_or(0) == 1 {
                "yes"
            } else {
                "no"
            };
...
fn render_visibility_table(headers: &[&str], items: &[Value], columns: &[&str]) -> String {
    ...
    let rows: Vec<Vec<String>> = items
        ...
        .map(|item| {
            columns
                .iter()
                .map(|c| escape_cell(&id_field(item, c)))
                .collect()
        })
        .collect();
    table(headers, &rows)
...
impl GlpiServer {
    ...
    pub async fn list_kb_articles(
        &self,
        Parameters(params): Parameters<ListKbArticlesParams>,
        ...
        let range = format!(
            "{}-{}",
            params.range_start,
            params.range_start + effective_limit - 1
        );
    ...
    #[rmcp::tool(
        description = "Get full details of a knowledge base article as Markdown, HTML stripped"
    )]
    pub async fn get_kb_article(
        &self,
        Parameters(params): Parameters<GetKbArticleParams>,
    ) -> Result<String, String> {
        let result = self
            .client
            ...
            .await
            .map_err(|e| e.to_string())?;
        let faq = if result.get("is_faq").and_then(Value::as_i64).unwrap_or(0) == 1 {
            "yes"
        } else {
            "no"
        };
    ...
    pub async fn search_kb_articles(
        &self,
        Parameters(params): Parameters<SearchKbArticlesParams>,
    ) -> Result<String, String> {
        let name_field = self
            .client
            .resolve_search_field_id("KnowbaseItem", "name", "6")
            .await;
        let answer_field = self
            .client
            .resolve_search_field_id("KnowbaseItem", "answer", "7")
            .await;
        let range = format!(
            "{}-{}",
            params.range_start,
    ...
    pub async fn create_kb_article(
        &self,
        Parameters(params): Parameters<CreateKbArticleParams>,
    ) -> Result<String, String> {
        let mut input = json!({
            "name": params.name,
    ...
    pub async fn update_kb_article(
        &self,
        Parameters(params): Parameters<UpdateKbArticleParams>,
    ) -> Result<String, String> {
        self.client
            .put(
                &format!("/KnowbaseItem/{}", params.article_id),
    ...
    pub async fn list_kb_categories(&self) -> Result<String, String> {
        ...
        let rows: Vec<Vec<String>> = items
            ...
            .map(|cat| {
                vec![
                    id_field(cat, "id"),
                    escape_cell(&str_field(cat, "completename")),
                ]
            })
            .collect();
        Ok(format!(
    ...
    pub async fn get_kb_article_visibility(
        &self,
        Parameters(params): Parameters<GetKbArticleVisibilityParams>,
    ) -> Result<String, String> {
        let id = params.article_id;
        let profiles = self
            .client
            .get(&format!("/KnowbaseItem/{id}/KnowbaseItem_Profile"), None)
            .await
            .map_err(|e| e.to_string())?;
        let groups = self
            .client
            .get(&format!("/KnowbaseItem/{id}/KnowbaseItem_Group"), None)
            .await
            .map_err(|e| e.to_string())?;
        let users = self
            .client
            .get(&format!("/KnowbaseItem/{id}/KnowbaseItem_User"), None)
            .await
            .map_err(|e| e.to_string())?;
        let entities = self
            .client
            .get(&format!("/KnowbaseItem/{id}/Entity_KnowbaseItem"), None)
            .await
            .map_err(|e| e.to_string())?;

        let profiles_md = render_visibility_table(
            &["ID", "Profile ID", "Entity"],
            &into_array(profiles),
            &["id", "profiles_id", "entities_id"],
        );
        let groups_md = render_visibility_table(
            &["ID", "Group ID", "Entity"],
            &into_array(groups),
            &["id", "groups_id", "entities_id"],
        );
        let users_md = render_visibility_table(
            &["ID", "User ID", "Entity"],
            &into_array(users),
            &["id", "users_id", "entities_id"],
        );
        let entities_md = render_visibility_table(
            &["ID", "Entity ID"],
            &into_array(entities),
            &["id", "entities_id"],
        );
    ...
    pub async fn add_kb_article_visibility_profile(
        &self,
        Parameters(params): Parameters<AddKbVisibilityProfileParams>,
    ) -> Result<String, String> {
        self.client
            .post(
            ...
            .map_err(|e| e.to_string())?;
        Ok(format!(
            "Profile #{} visibility added to KB article #{}.",
            params.profiles_id, params.article_id
        ))
    }

    #[rmcp::tool(description = "Add a group to a KB article's visibility rules")]
    pub async fn add_kb_article_visibility_group(
        &self,
        Parameters(params): Parameters<AddKbVisibilityGroupParams>,
    ) -> Result<String, String> {
        self.client
            .post(
                "/KnowbaseItem_Group",
            ...
            .map_err(|e| e.to_string())?;
        Ok(format!(
            "Group #{} visibility added to KB article #{}.",
            params.groups_id, params.article_id
        ))
    }

    #[rmcp::tool(description = "Update a KB article's profile-based visibility rule")]
    pub async fn update_kb_article_visibility_profile(
        &self,
        Parameters(params): Parameters<UpdateKbVisibilityParams>,
    ) -> Result<String, String> {
        self.client
            .put(
                &format!("/KnowbaseItem_Profile/{}", params.visibility_id),
                &json!({ "input": params.update_fields }),
            )
            .await
            .map_err(|e| e.to_string())?;
        Ok(format!(
            "Profile visibility rule #{} updated.",
            params.visibility_id
        ))
    }

    #[rmcp::tool(description = "Update a KB article's group-based visibility rule")]
    pub async fn update_kb_article_visibility_group(
        &self,
        Parameters(params): Parameters<UpdateKbVisibilityParams>,
    ) -> Result<String, String> {
        self.client
            .put(
                &format!("/KnowbaseItem_Group/{}", params.visibility_id),
                &json!({ "input": params.update_fields }),
            )
            .await
            .map_err(|e| e.to_string())?;
        Ok(format!(
            "Group visibility rule #{} updated.",
            params.visibility_id
        ))
    }
}
