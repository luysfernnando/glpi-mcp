...
impl GlpiServer {
    ...
    pub async fn list_tasks(
        ...
        let rows: Vec<Vec<String>> = items
            ...
            .map(|task| {
                ...
                let status = lookup(
                    &self.labels.task_status,
                    task.get("state").and_then(Value::as_i64),
                    self.labels.unknown,
                )
                .to_string();
                let assignee = task
                ...
                let duration = task
                    ...
                    .unwrap_or_default();
                let content = cell(
                    task.get("content").and_then(Value::as_str).unwrap_or(""),
                    100,
                );
        ...
        Ok(format!(
            ...
            items.len(),
            table(
                &["ID", "Status", "Assigned to", "Duration", "Content"],
                &rows
            )
        ))
    }
    ...
    pub async fn update_task(
        ...
        self.client
            .put(
                &format!("/TicketTask/{}", params.task_id),
